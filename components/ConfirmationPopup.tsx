...
function getCredentialDetails(credential: VotingProcess): {
  ...
  if (
    (credential.contractpoll &&
      credential.contractpoll.includes('ProtocolGuild on-chain')) ||
  ...
  if (
    (credential.contractpoll &&
      credential.contractpoll.includes('EthHolding on-chain')) ||
...
const ConfirmationPopup: React.FC<ConfirmationPopupProps> = ({
  ...
  const getStatusIcon = (credentialId: string) => {
    ...
    if (process?.status === 'success') {
      ...
      return process ? (
        <img src={getCredentialDetails(process).imgSrc} alt="Credential" />
      ) : null;
    }
  };
  ...
  const getConfirmedButton = () => {
    ...
    if (!allConfirmed) {
      ...
      return (
        <button className={styles.vote_btn} onClick={onClose}>
          <HiCheck />
          <span>Done</span>
        </button>
      );
