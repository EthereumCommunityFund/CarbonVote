...
const CreatePollPage = () => {
  ...
  const createNewPoll = async () => {
    ...
    if (!motionTitle || motionTitle.length > 2000) {
      console.error('Invalid motion title');
      return;
    }
    if (!motionDescription || motionDescription.length > 500) {
      console.error('Invalid motion description');
      return;
    }
    const durationInMinutes = convertToMinutes(timeLimit); // Assuming this returns a positive integer
    ...
    try {
      console.log(motionTitle, motionDescription, durationInMinutes, numOptions);
      if (pollContract) {
        const tx = await pollContract.createPoll(motionTitle, motionDescription, durationInMinutes, numOptions);
        await tx.wait();
  ...
  // Function to parse a string like "2h 30m" and convert it to minutes
        ...
        <div className="bg-white flex flex-col gap-2.5 rounded-2xl p-5 ">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1">
              ...
              <select className="flex w-full text-black outline-none rounded-lg py-2.5 pr-3 pl-2.5 bg-inputField gap-2.5 items-center border border-white/10 border-opacity-10" title="Voting Method">
