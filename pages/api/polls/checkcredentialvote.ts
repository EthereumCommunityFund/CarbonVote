...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    if (isEthHolding === 'true') {
      ...
      const optionsDescriptions = await Promise.all(
        optionIds.map(async (id) => {
          ...
          return {
            id,
            description: data?.option_description || 'No description',
          };
        })
      );

      const descriptionLookup = optionsDescriptions.reduce<{
        [key: string]: string;
      }>((acc, { id, description }) => {
        acc[id] = description;
        return acc;
      ...
      const transformedData: VoteData[] = votesData.reduce(
        (acc: VoteData[], { option_id, voter_identifier }) => {
    ...
    `
      ...
      const transformedData: VoteData[] = votesData.reduce(
        (acc: VoteData[], { option_id, vote_credential }) => {
          const description = descriptionLookup[option_id] || 'No description';
          let entry = acc.find(
            (x) => x.id === option_id && x.credential === vote_credential
          );
          if (entry) {
            entry.votes += 1;
          } else {
