...
async function verifySignature({
  ...
  voter_identifier,
  signature,
}: VerifySignatureInput): Promise<string | undefined> {
  try {
    let signerAddress;
    const message = generateMessage(poll_id, option_id, voter_identifier);
    const domain = {
      ...EIP712_DOMAIN,
      chainId: !IS_PROD ? 11155111 : 1,
    };
    signerAddress = verifyTypedData(domain, EIP712_TYPE, message, signature);
...
async function checkPOAPOwnership({
  pollData,
  voter_identifier,
}: CheckPOAPOwnershipInput): Promise<void> {
...
const containsCredentialById = (
  requiredCredentials: any[] | null,
  credId: string
) => {
...
const createVote = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (containsSingature) {
      signerAddress = await verifySignature({
        poll_id,
        option_id,
        voter_identifier,
        signature,
      ...
      if (vote_credential === CREDENTIALS.ProtocolGuildMember.id) {
        if (!ProtocolGuildMembershipList.includes(voter_identifier)) {
          res
            .status(403)
            .json({ error: "You don't qualify to vote for this poll." });
        }
      }
      ...
      if (vote_credential === CREDENTIALS.GitcoinPassport.id) {
        if (gitscore < pollData.gitcoin_score)
          res.status(403).json({
            error: "You don't have enough score to vote for this poll.",
          });
      }

      // Whitelisted Addresses verification
      if (vote_credential === CREDENTIALS.WhitelistedAddresses.id) {
        try {
          const normalizedVoterAddress = getAddress(voter_identifier);

          if (
            !pollData?.white_list ||
            !pollData.white_list.some(
              (addr) =>
                addr.toLowerCase() === normalizedVoterAddress.toLowerCase()
            )
          ) {
            res
              .status(403)
              .json({ error: 'You are not in the whitelist for this poll.' });
            return;
          }
        } catch (e) {
