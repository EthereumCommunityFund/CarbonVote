...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!voter_identifier || !eventId)
      return res
        .status(404)
        .send(
          'An identifier for the voter and an EventId are required for this'
        );

    const response = await getPoapOwnership(
      poapApiKey,
      voter_identifier,
      eventId
    );
    ...
    return res
      .status(500)
      .send(`Server error occurred while  looking for POAPs. Error: ${error}`);
  }
}
